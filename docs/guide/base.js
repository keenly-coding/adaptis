...
enumerable:!0}):target,mod));var require_color_name=__commonJS({"node_modules/color-name/index.js"(exports,module){
...
"zeroArray");module.exports=Color4}});function sleepSync(ms){let end=new Date().getTime()+ms,time=new Date().getTime();
...
this),this._adapter}static cssProcess(css6){return css6}static set css(css6){this.
adapter.style=css6}static get css(){return this.adapter.style}static get tagName(){
return this.adapter.tagName}static addStyle(css6){this.adapter.addStyle(css6)}static define(tagName){
...
__name(this,"Adapter")}static cssProcess(css6){return stylis(css6)}};var import_color=__toESM(require_color(),1);var css=String.raw;function bgColor(color2){return css`
...
filter: drop-shadow(0 0 ${level*2}px ${color2});`}__name(lift,"lift");function pxToRem(px){
px=parseFloat(px);let rem1=parseFloat(getComputedStyle(document.documentElement).
fontSize);return`${px/rem1}rem`}__name(pxToRem,"pxToRem");var aspectRatio=__name(
(ratio="1/1")=>css`
    height: auto;
    aspect-ratio: ${ratio};
    ...
    `.trim(),"aspectRatio");var color={blue:"#3584e4",green:"#33d17a",yellow:"#f6d32d",orange:"#ff7800",red:"\
#e01b24",purple:"#9141ac",violet:"#9141ac",brown:"#986a44",light:"#deddda",dark:"\
#3d3846"};function menuStyle(param={}){return`
    div[class] {all: unset};
    ...
    `.trim()}__name(menuStyle,"menuStyle");var Menu=class extends Adapter{static{
__name(this,"Menu")}static{this.css=menuStyle()}static{this.style=menuStyle}constructor(){
super(),this.addEventListener("click",e=>{let target=e.target;if(target.tagName.
toLowerCase()==="summary"){e.preventDefault();return}if(!target.classList.contains(
"toggle"))return;e.preventDefault();let el_details=target.closest(`${this.tagName}\
 summary`);el_details&&(el_details=el_details.parentElement,el_details.open?this.
close(el_details):this.open(el_details))})}open(el_details){el_details||(el_details=
this.querySelector("details")),el_details.classList.add("open");let el_parentContainer=el_details.
parentElement.closest(`${this.tagName} div.container`);el_parentContainer&&(el_parentContainer.
style.height="auto");let el_container=el_details.querySelector("div.container"),
height=pxToRem(getComputedStyle(el_container).height),parentDetailsElement=el_details.
parentElement.closest(`${this.tagName} details`);for(;parentDetailsElement;)parentDetailsElement.
open||this.open(parentDetailsElement),parentDetailsElement=parentDetailsElement.
parentElement.closest("details");return el_details.open=!0,el_container.style.height=
"0",setTimeout(()=>{el_container.style.height=height},0),this}expand(el_details){
let childDetailsElement=el_details.querySelector("details");for(;childDetailsElement;)
this.open(childDetailsElement),childDetailsElement=childDetailsElement.querySelector(
"details")}close(el_details){el_details.classList.remove("open");let el_container=el_details.
querySelector("div.container");el_container.style.height=pxToRem(getComputedStyle(
el_container).height),setTimeout(()=>{el_container.style.height="0"},0),setTimeout(
()=>{el_details.open=!1,el_container.style.height="auto"},300)}collapse(el_details){
this.close(el_details);let childDetailsElement=el_details.querySelector("details");
for(;childDetailsElement;)this.close(childDetailsElement),childDetailsElement=childDetailsElement.
querySelector("details")}};var css2=String.raw,Menu2=class extends Menu{static{__name(this,"Menu")}static{this.
css=css2`
    padding-top: 40dvh;
    padding-bottom: 5rem;
    a {
...
this.innerHTML;this.render()}static{__name(this,"Button")}static{this.css=css3`
  ...
  `.trim(),"sidebarStyle"),Sidebar=class extends Adapter{static{__name(this,"Sid\
...
show()}};var css5=String.raw,sideBarStyle=css5`
  ...
  filter: drop-shadow(2px 2px 4px ${(0,import_color3.default)(color.dark).alpha(
.8).string()});
...
target;["A","H1","H2"].includes(target.tagName)&&this.onLinkClick()});let mql=window.
matchMedia("screen and (max-width: 1200px)");setTimeout(()=>{this.onMediaQueryChange(
mql)},750),setTimeout(()=>{mql.addEventListener("change",()=>{this.onMediaQueryChange(
mql)})},750)}onMediaQueryChange(mql){mql.matches?this.hide():this.show()}onLinkClick(){
window.innerWidth>1200||setTimeout(()=>{this.hide()},500)}show(){super.show(),this.
dispatchEvent(new CustomEvent("show"))}hide(){super.hide(),this.dispatchEvent(new CustomEvent(
"hide"))}};var Container=class extends Adapter{static{__name(this,"Container")}static{this.
css=`& {
        all: unset;
        display: block;
        box-sizing: border-box;
    ...
    }`}constructor(){super();let el_sidebar=document.querySelector("el-sidebar");
setTimeout(()=>{this.css=this.defaultStyle()},0),el_sidebar.addEventListener("hi\
de",()=>{this.addStyle("margin: unset;")}),el_sidebar.addEventListener("show",()=>{
this.css=this.defaultStyle()})}defaultStyle(){let el_sidebar=document.querySelector(
"el-sidebar");return`& {
