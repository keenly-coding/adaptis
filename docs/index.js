...
sort().join("")]=model;var limiters={};function Color3(object,model){if(!(this instanceof
Color3))return new Color3(object,model);if(model&&model in skippedModels&&(model=
...
Color3)this.model=object.model,this.color=[...object.color],this.valpha=object.valpha;else if(typeof object==
...
Color3,"Color");Color3.prototype={toString(){return this.string()},toJSON(){return this[this.
...
this.valpha),rgb},round(places){return places=Math.max(places||0,0),new Color3([
...this.color.map(roundToPlace(places)),this.valpha],this.model)},alpha(value){return value!==
void 0?new Color3([...this.color,Math.max(0,Math.min(1,value))],this.model):this.
...
2),keyword(value){return value!==void 0?new Color3(value):convert[this.model].keyword(
this.color)},hex(value){return value!==void 0?new Color3(value):colorString.to.hex(
this.rgb().round().color)},hexa(value){if(value!==void 0)return new Color3(value);
...
let rgb=this.rgb().color,value=rgb[0]*.3+rgb[1]*.59+rgb[2]*.11;return Color3.rgb(
...
w*a))+1)/2,w2=1-w1;return Color3.rgb(w1*color1.red()+w2*color2.red(),w1*color1.green()+
...
continue;let{channels}=convert[model];Color3.prototype[model]=function(...args){
return this.model===model?new Color3(this):args.length>0?new Color3(args,model):
new Color3([...assertArray(convert[this.model][model].raw(this.color)),this.valpha],
model)},Color3[model]=function(...args){let color2=args[0];return typeof color2==
"number"&&(color2=zeroArray(args,channels)),new Color3(color2,model)}}function roundTo(number,places){
...
"zeroArray");module.exports=Color3}});function sleepSync(ms){let end=new Date().getTime()+ms,time=new Date().getTime();
...
stringify,"stringify");function stylis(css3){return serialize(compile(css3),stringify)}__name(stylis,"s\
...
this),this._adapter}static cssProcess(css3){return css3}static set css(css3){this.
adapter.style=css3}static get css(){return this.adapter.style}static get tagName(){
return this.adapter.tagName}static addStyle(css3){this.adapter.addStyle(css3)}static define(tagName){
this.adapter.define(tagName)}set css(css3){this.classList.add(this._adapter.uuid);
let processedCss=this._adapter._class.cssProcess(`${this.tagName}.${this._adapter.
objectClassSelector} { ${css3} }`);this._adapter.cssStyleSheet.replaceSync(processedCss)}get css(){
let css3=this.getAttribute("css")||"";if(css3)return css3;for(let rule of this._adapter.
cssStyleSheet.cssRules)css3+=rule.cssText+`
`;return css3}addStyle(css3){this.classList.add(this._adapter.uuid);let processedCss=this.
_adapter._class.cssProcess(`${this.tagName}.${this._adapter.objectClassSelector}\
 { ${css3} }`);this._adapter.cssStyleSheet.replaceSync(`
        ${this.css}
        ${processedCss}
      `)}connectedCallback(){super.connectedCallback&&super.connectedCallback();
let css3=this.getAttribute("css");css3&&(this.css=css3);let rootNode=this.getRootNode();
...
__name(this,"Adapter")}static cssProcess(css3){return stylis(css3)}};var import_color2=__toESM(require_color(),1);var import_color=__toESM(require_color(),1);var css=String.raw;function bgColor(color2){return css`
...
filter: drop-shadow(0 0 ${level*2}px ${color2});`}__name(lift,"lift");var color={blue:"#3584e4",green:"#33d17a",yellow:"#f6d32d",orange:"#ff7800",red:"\
#e01b24",purple:"#9141ac",violet:"#9141ac",brown:"#986a44",light:"#deddda",dark:"\
#3d3846"};var css2=String.raw,buttonStyle=__name(color2=>css2`
    display: inline-flex;
    flex-wrap: wrap;
...
this.innerHTML;this.render()}static{__name(this,"Button")}static{this.css=css2`
  ...
  `}render(){this.innerHTML=`<button>${this.initialHTML}</button>`}};var Heading=class extends Adapter{static{__name(this,"Heading")}};Heading.css=`
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    text-align: center;
    ...
    & h1 {
        ...
        background: linear-gradient(90deg, rgba(49,153,154,1) 0%, rgba(194,125,2\
55,1) 50%, rgba(255,127,0,1) 100%);
...
`;var HeadingButtons=class extends Adapter{static{__name(this,"HeadingButtons")}};
HeadingButtons.css=`
    button {
    margin: 1rem 1rem;
    & el-icon {
