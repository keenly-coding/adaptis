...
mode):mode}__name(expandOrCloneMode,"expandOrCloneMode");var version="11.11.0",HTMLInjectionError=class extends Error{static{
...
NO_MATCH)return processed}if(match.type==="illegal"&&lexeme==="")return modeBuffer+=
`
`,1;if(iterations>1e5&&iterations>match.index*3)throw new Error("potential infin\
ite loop, way more iterations than matches");return modeBuffer+=lexeme,lexeme.length}
__name(processLexeme,"processLexeme");let language=getLanguage(languageName);if(!language)
throw error(LANGUAGE_NOT_FOUND.replace("{}",languageName)),new Error('Unknown la\
nguage: "'+languageName+'"');let md=compileLanguage(language),result="",top=continuation||
md,continuations={},emitter=new options.__emitter(options);processContinuations();
let modeBuffer="",relevance=0,index=0,iterations=0,resumeScanAtSamePosition=!1;try{
if(language.__emitTokens)language.__emitTokens(codeToHighlight,emitter);else{for(top.
matcher.considerAll();;){iterations++,resumeScanAtSamePosition?resumeScanAtSamePosition=
!1:top.matcher.considerAll(),top.matcher.lastIndex=index;let match=top.matcher.exec(
codeToHighlight);if(!match)break;let beforeMatch=codeToHighlight.substring(index,
match.index),processedCount=processLexeme(beforeMatch,match);index=match.index+processedCount}
processLexeme(codeToHighlight.substring(index))}return emitter.finalize(),result=
emitter.toHTML(),{language:languageName,value:result,relevance,illegal:!1,_emitter:emitter,
_top:top}}catch(err){if(err.message&&err.message.includes("Illegal"))return{language:languageName,
value:escape(codeToHighlight),illegal:!0,relevance:0,_illegalBy:{message:err.message,
index,context:codeToHighlight.slice(index-100,index+100),mode:err.mode,resultSoFar:result},
_emitter:emitter};if(SAFE_MODE)return{language:languageName,value:escape(codeToHighlight),
illegal:!1,relevance:0,errorRaised:err,_emitter:emitter,_top:top};throw err}}__name(
_highlight,"_highlight");function justTextHighlightResult(code){let result={value:escape(
code),illegal:!1,relevance:0,_top:PLAINTEXT_LANGUAGE,_emitter:new options.__emitter(
options)};return result._emitter.addText(code),result}__name(justTextHighlightResult,
"justTextHighlightResult");function highlightAuto(code,languageSubset){languageSubset=
languageSubset||options.languages||Object.keys(languages);let plaintext=justTextHighlightResult(
...
"initHighlightingOnLoad");let wantsHighlight=!1;function highlightAll(){function boot(){
highlightAll()}if(__name(boot,"boot"),document.readyState==="loading"){wantsHighlight||
window.addEventListener("DOMContentLoaded",boot,!1),wantsHighlight=!0;return}document.
querySelectorAll(options.cssSelector).forEach(highlightElement)}__name(highlightAll,
"highlightAll");function registerLanguage(languageName,languageDefinition){let lang=null;
try{lang=languageDefinition(hljs)}catch(error$1){if(error("Language definition f\
or '{}' could not be registered.".replace("{}",languageName)),SAFE_MODE)error(error$1);else
throw error$1;lang=PLAINTEXT_LANGUAGE}lang.name||(lang.name=languageName),languages[languageName]=
lang,lang.rawDefinition=languageDefinition.bind(null,hljs),lang.aliases&&registerAliases(
lang.aliases,{languageName})}__name(registerLanguage,"registerLanguage");function unregisterLanguage(languageName){
delete languages[languageName];for(let alias of Object.keys(aliases))aliases[alias]===
languageName&&delete aliases[alias]}__name(unregisterLanguage,"unregisterLanguag\
e");function listLanguages(){return Object.keys(languages)}__name(listLanguages,
"listLanguages");function getLanguage(name){return name=(name||"").toLowerCase(),
languages[name]||languages[aliases[name]]}__name(getLanguage,"getLanguage");function registerAliases(aliasList,{
languageName}){typeof aliasList=="string"&&(aliasList=[aliasList]),aliasList.forEach(
alias=>{aliases[alias.toLowerCase()]=languageName})}__name(registerAliases,"regi\
sterAliases");function autoDetection(name){let lang=getLanguage(name);return lang&&
!lang.disableAutodetect}__name(autoDetection,"autoDetection");function upgradePluginAPI(plugin){
plugin["before:highlightBlock"]&&!plugin["before:highlightElement"]&&(plugin["be\
fore:highlightElement"]=data=>{plugin["before:highlightBlock"](Object.assign({block:data.
el},data))}),plugin["after:highlightBlock"]&&!plugin["after:highlightElement"]&&
(plugin["after:highlightElement"]=data=>{plugin["after:highlightBlock"](Object.assign(
{block:data.el},data))})}__name(upgradePluginAPI,"upgradePluginAPI");function addPlugin(plugin){
upgradePluginAPI(plugin),plugins.push(plugin)}__name(addPlugin,"addPlugin");function removePlugin(plugin){
...
rt","from","export","extends","using"],LITERALS=["true","false","null","undefine\
d","NaN","Infinity"],TYPES=["Object","Function","Boolean","Symbol","Math","Date",
"Number","BigInt","String","RegExp","Array","Float32Array","Float64Array","Int8A\
rray","Uint8Array","Uint8ClampedArray","Int16Array","Int32Array","Uint16Array","\
Uint32Array","BigInt64Array","BigUint64Array","Set","Map","WeakSet","WeakMap","A\
rrayBuffer","SharedArrayBuffer","Atomics","DataView","JSON","Promise","Generator",
"GeneratorFunction","AsyncFunction","Reflect","Proxy","Intl","WebAssembly"],ERROR_TYPES=[
...
contains:[]},HTML_TEMPLATE={begin:".?html`",end:"",starts:{end:"`",returnEnd:!1,
contains:[hljs.BACKSLASH_ESCAPE,SUBST],subLanguage:"xml"}},CSS_TEMPLATE={begin:"\
.?css`",end:"",starts:{end:"`",returnEnd:!1,contains:[hljs.BACKSLASH_ESCAPE,SUBST],
subLanguage:"css"}},GRAPHQL_TEMPLATE={begin:".?gql`",end:"",starts:{end:"`",returnEnd:!1,
contains:[hljs.BACKSLASH_ESCAPE,SUBST],subLanguage:"graphql"}},TEMPLATE_STRING={
className:"string",begin:"`",end:"`",contains:[hljs.BACKSLASH_ESCAPE,SUBST]},COMMENT2={
className:"comment",variants:[hljs.COMMENT(/\/\*\*(?!\/)/,"\\*/",{relevance:0,contains:[
{begin:"(?=@[A-Za-z]+)",relevance:0,contains:[{className:"doctag",begin:"@[A-Za-\
z]+"},{className:"type",begin:"\\{",end:"\\}",excludeEnd:!0,excludeBegin:!0,relevance:0},
{className:"variable",begin:IDENT_RE$1+"(?=\\s*(-)|$)",endsParent:!0,relevance:0},
{begin:/(?=[^\n])\s/,relevance:0}]}]}),hljs.C_BLOCK_COMMENT_MODE,hljs.C_LINE_COMMENT_MODE]},
SUBST_INTERNALS=[hljs.APOS_STRING_MODE,hljs.QUOTE_STRING_MODE,HTML_TEMPLATE,CSS_TEMPLATE,
GRAPHQL_TEMPLATE,TEMPLATE_STRING,{match:/\$\d+/},NUMBER];SUBST.contains=SUBST_INTERNALS.
concat({begin:/\{/,end:/\}/,keywords:KEYWORDS$1,contains:["self"].concat(SUBST_INTERNALS)});
let SUBST_AND_COMMENTS=[].concat(COMMENT2,SUBST.contains),PARAMS_CONTAINS=SUBST_AND_COMMENTS.
concat([{begin:/(\s*)\(/,end:/\)/,keywords:KEYWORDS$1,contains:["self"].concat(SUBST_AND_COMMENTS)}]),
PARAMS={className:"params",begin:/(\s*)\(/,end:/\)/,excludeBegin:!0,excludeEnd:!0,
keywords:KEYWORDS$1,contains:PARAMS_CONTAINS},CLASS_OR_EXTENDS={variants:[{match:[
/class/,/\s+/,IDENT_RE$1,/\s+/,/extends/,/\s+/,regex.concat(IDENT_RE$1,"(",regex.
concat(/\./,IDENT_RE$1),")*")],scope:{1:"keyword",3:"title.class",5:"keyword",7:"\
title.class.inherited"}},{match:[/class/,/\s+/,IDENT_RE$1],scope:{1:"keyword",3:"\
title.class"}}]},CLASS_REFERENCE={relevance:0,match:regex.either(/\bJSON/,/\b[A-Z][a-z]+([A-Z][a-z]*|\d)*/,
/\b[A-Z]{2,}([A-Z][a-z]+|\d)+([A-Z][a-z]*)*/,/\b[A-Z]{2,}[a-z]+([A-Z][a-z]+|\d)*([A-Z][a-z]*)*/),
className:"title.class",keywords:{_:[...TYPES,...ERROR_TYPES]}},USE_STRICT={label:"\
use_strict",className:"meta",relevance:10,begin:/^\s*['"]use (strict|asm)['"]/},
FUNCTION_DEFINITION={variants:[{match:[/function/,/\s+/,IDENT_RE$1,/(?=\s*\()/]},
{match:[/function/,/\s*(?=\()/]}],className:{1:"keyword",3:"title.function"},label:"\
func.def",contains:[PARAMS],illegal:/%/},UPPER_CASE_CONSTANT={relevance:0,match:/\b[A-Z][A-Z_0-9]+\b/,
className:"variable.constant"};function noneOf(list){return regex.concat("(?!",list.
join("|"),")")}__name(noneOf,"noneOf");let FUNCTION_CALL={match:regex.concat(/\b/,
noneOf([...BUILT_IN_GLOBALS,"super","import"].map(x=>`${x}\\s*\\(`)),IDENT_RE$1,
regex.lookahead(/\s*\(/)),className:"title.function",relevance:0},PROPERTY_ACCESS={
...
{scope:"attr",match:IDENT_RE$1+regex.lookahead(":"),relevance:0},FUNCTION_VARIABLE,
...
skip:!0},{begin:/(\s*)\(/,end:/\)/,excludeBegin:!0,excludeEnd:!0,keywords:KEYWORDS$1,
...
mport","from","export","extends","using"],LITERALS2=["true","false","null","unde\
fined","NaN","Infinity"],TYPES2=["Object","Function","Boolean","Symbol","Math","\
Date","Number","BigInt","String","RegExp","Array","Float32Array","Float64Array",
"Int8Array","Uint8Array","Uint8ClampedArray","Int16Array","Int32Array","Uint16Ar\
ray","Uint32Array","BigInt64Array","BigUint64Array","Set","Map","WeakSet","WeakM\
ap","ArrayBuffer","SharedArrayBuffer","Atomics","DataView","JSON","Promise","Gen\
erator","GeneratorFunction","AsyncFunction","Reflect","Proxy","Intl","WebAssembl\
y"],ERROR_TYPES2=["Error","EvalError","InternalError","RangeError","ReferenceErr\
or","SyntaxError","TypeError","URIError"],BUILT_IN_GLOBALS2=["setInterval","setT\
imeout","clearInterval","clearTimeout","require","exports","eval","isFinite","is\
NaN","parseFloat","parseInt","decodeURI","decodeURIComponent","encodeURI","encod\
eURIComponent","escape","unescape"],BUILT_IN_VARIABLES2=["arguments","this","sup\
er","console","window","document","localStorage","sessionStorage","module","glob\
al"],BUILT_INS2=[].concat(BUILT_IN_GLOBALS2,TYPES2,ERROR_TYPES2);function javascript2(hljs){
let regex=hljs.regex,hasClosingTag=__name((match,{after})=>{let tag="</"+match[0].
slice(1);return match.input.indexOf(tag,after)!==-1},"hasClosingTag"),IDENT_RE$1=IDENT_RE2,
FRAGMENT={begin:"<>",end:"</>"},XML_SELF_CLOSING=/<[A-Za-z0-9\\._:-]+\s*\/>/,XML_TAG={
...
if(nextChar==="<"||nextChar===","){response.ignoreMatch();return}nextChar===">"&&
(hasClosingTag(match,{after:afterMatchIndex})||response.ignoreMatch());let m,afterMatch=match.
input.substring(afterMatchIndex);if(m=afterMatch.match(/^\s*=/)){response.ignoreMatch();
return}if((m=afterMatch.match(/^\s+extends\s+/))&&m.index===0){response.ignoreMatch();
return}}},KEYWORDS$1={$pattern:IDENT_RE2,keyword:KEYWORDS2,literal:LITERALS2,built_in:BUILT_INS2,
"variable.language":BUILT_IN_VARIABLES2},decimalDigits="[0-9](_?[0-9])*",frac=`\\\
.(${decimalDigits})`,decimalInteger="0|[1-9](_?[0-9])*|0[0-7]*[89][0-9]*",NUMBER={
className:"number",variants:[{begin:`(\\b(${decimalInteger})((${frac})|\\.)?|(${frac}\
))[eE][+-]?(${decimalDigits})\\b`},{begin:`\\b(${decimalInteger})\\b((${frac})\\b\
|\\.)?|(${frac})\\b`},{begin:"\\b(0|[1-9](_?[0-9])*)n\\b"},{begin:"\\b0[xX][0-9a-\
fA-F](_?[0-9a-fA-F])*n?\\b"},{begin:"\\b0[bB][0-1](_?[0-1])*n?\\b"},{begin:"\\b0[\
oO][0-7](_?[0-7])*n?\\b"},{begin:"\\b0[0-7]+n?\\b"}],relevance:0},SUBST={className:"\
subst",begin:"\\$\\{",end:"\\}",keywords:KEYWORDS$1,contains:[]},HTML_TEMPLATE={
begin:".?html`",end:"",starts:{end:"`",returnEnd:!1,contains:[hljs.BACKSLASH_ESCAPE,
SUBST],subLanguage:"xml"}},CSS_TEMPLATE={begin:".?css`",end:"",starts:{end:"`",returnEnd:!1,
contains:[hljs.BACKSLASH_ESCAPE,SUBST],subLanguage:"css"}},GRAPHQL_TEMPLATE={begin:"\
.?gql`",end:"",starts:{end:"`",returnEnd:!1,contains:[hljs.BACKSLASH_ESCAPE,SUBST],
subLanguage:"graphql"}},TEMPLATE_STRING={className:"string",begin:"`",end:"`",contains:[
hljs.BACKSLASH_ESCAPE,SUBST]},COMMENT2={className:"comment",variants:[hljs.COMMENT(
/\/\*\*(?!\/)/,"\\*/",{relevance:0,contains:[{begin:"(?=@[A-Za-z]+)",relevance:0,
contains:[{className:"doctag",begin:"@[A-Za-z]+"},{className:"type",begin:"\\{",
end:"\\}",excludeEnd:!0,excludeBegin:!0,relevance:0},{className:"variable",begin:IDENT_RE$1+
"(?=\\s*(-)|$)",endsParent:!0,relevance:0},{begin:/(?=[^\n])\s/,relevance:0}]}]}),
hljs.C_BLOCK_COMMENT_MODE,hljs.C_LINE_COMMENT_MODE]},SUBST_INTERNALS=[hljs.APOS_STRING_MODE,
hljs.QUOTE_STRING_MODE,HTML_TEMPLATE,CSS_TEMPLATE,GRAPHQL_TEMPLATE,TEMPLATE_STRING,
{match:/\$\d+/},NUMBER];SUBST.contains=SUBST_INTERNALS.concat({begin:/\{/,end:/\}/,
keywords:KEYWORDS$1,contains:["self"].concat(SUBST_INTERNALS)});let SUBST_AND_COMMENTS=[].
concat(COMMENT2,SUBST.contains),PARAMS_CONTAINS=SUBST_AND_COMMENTS.concat([{begin:/(\s*)\(/,
end:/\)/,keywords:KEYWORDS$1,contains:["self"].concat(SUBST_AND_COMMENTS)}]),PARAMS={
className:"params",begin:/(\s*)\(/,end:/\)/,excludeBegin:!0,excludeEnd:!0,keywords:KEYWORDS$1,
...
"super","import"].map(x=>`${x}\\s*\\(`)),IDENT_RE$1,regex.lookahead(/\s*\(/)),className:"\
title.function",relevance:0},PROPERTY_ACCESS={begin:regex.concat(/\./,regex.lookahead(
regex.concat(IDENT_RE$1,/(?![0-9A-Za-z$_(])/))),end:IDENT_RE$1,excludeBegin:!0,keywords:"\
prototype",className:"property",relevance:0},GETTER_OR_SETTER={match:[/get|set/,
/\s+/,IDENT_RE$1,/(?=\()/],className:{1:"keyword",3:"title.function"},contains:[
{begin:/\(\)/},PARAMS]},FUNC_LEAD_IN_RE="(\\([^()]*(\\([^()]*(\\([^()]*\\)[^()]*)*\\)\
[^()]*)*\\)|"+hljs.UNDERSCORE_IDENT_RE+")\\s*=>",FUNCTION_VARIABLE={match:[/const|var|let/,
/\s+/,IDENT_RE$1,/\s*/,/=\s*/,/(async\s*)?/,regex.lookahead(FUNC_LEAD_IN_RE)],keywords:"\
...
NUMBER,CLASS_REFERENCE,{scope:"attr",match:IDENT_RE$1+regex.lookahead(":"),relevance:0},
...
relevance:0},{match:[/\bconstructor(?=\s*\()/],className:{1:"title.function"},contains:[
PARAMS]},FUNCTION_CALL,UPPER_CASE_CONSTANT,CLASS_OR_EXTENDS,GETTER_OR_SETTER,{match:/\$[(.]/}]}}
__name(javascript2,"javascript");function typescript(hljs){let regex=hljs.regex,
tsLanguage=javascript2(hljs),IDENT_RE$1=IDENT_RE2,TYPES3=["any","void","number",
"boolean","string","object","never","symbol","bigint","unknown"],NAMESPACE={begin:[
/namespace/,/\s+/,hljs.IDENT_RE],beginScope:{1:"keyword",3:"title.class"}},INTERFACE={
beginKeywords:"interface",end:/\{/,excludeEnd:!0,keywords:{keyword:"interface ex\
tends",built_in:TYPES3},contains:[tsLanguage.exports.CLASS_REFERENCE]},USE_STRICT={
className:"meta",relevance:10,begin:/^\s*['"]use strict['"]/},TS_SPECIFIC_KEYWORDS=[
"type","interface","public","private","protected","implements","declare","abstra\
ct","readonly","enum","override","satisfies"],KEYWORDS$1={$pattern:IDENT_RE2,keyword:KEYWORDS2.
concat(TS_SPECIFIC_KEYWORDS),literal:LITERALS2,built_in:BUILT_INS2.concat(TYPES3),
"variable.language":BUILT_IN_VARIABLES2},DECORATOR={className:"meta",begin:"@"+IDENT_RE$1},
swapMode=__name((mode,label,replacement)=>{let indx=mode.contains.findIndex(m=>m.
label===label);if(indx===-1)throw new Error("can not find mode to replace");mode.
contains.splice(indx,1,replacement)},"swapMode");Object.assign(tsLanguage.keywords,
KEYWORDS$1),tsLanguage.exports.PARAMS_CONTAINS.push(DECORATOR);let ATTRIBUTE_HIGHLIGHT=tsLanguage.
contains.find(c=>c.scope==="attr"),OPTIONAL_KEY_OR_ARGUMENT=Object.assign({},ATTRIBUTE_HIGHLIGHT,
{match:regex.concat(IDENT_RE$1,regex.lookahead(/\s*\?:/))});tsLanguage.exports.PARAMS_CONTAINS.
push([tsLanguage.exports.CLASS_REFERENCE,ATTRIBUTE_HIGHLIGHT,OPTIONAL_KEY_OR_ARGUMENT]),
tsLanguage.contains=tsLanguage.contains.concat([DECORATOR,NAMESPACE,INTERFACE,OPTIONAL_KEY_OR_ARGUMENT]),
swapMode(tsLanguage,"shebang",hljs.SHEBANG()),swapMode(tsLanguage,"use_strict",USE_STRICT);
let functionDeclaration=tsLanguage.contains.find(m=>m.label==="func.def");return functionDeclaration.
relevance=0,Object.assign(tsLanguage,{name:"TypeScript",aliases:["ts","tsx","mts",
"cts"]}),tsLanguage}__name(typescript,"typescript");function xml(hljs){let regex=hljs.regex,TAG_NAME_RE=regex.concat(/[\p{L}_]/u,regex.
...
"MODES"),HTML_TAGS=["a","abbr","address","article","aside","audio","b","blockquo\
te","body","button","canvas","caption","cite","code","dd","del","details","dfn",
"div","dl","dt","em","fieldset","figcaption","figure","footer","form","h1","h2",
"h3","h4","h5","h6","header","hgroup","html","i","iframe","img","input","ins","k\
bd","label","legend","li","main","mark","menu","nav","object","ol","optgroup","o\
ption","p","picture","q","quote","samp","section","select","source","span","stro\
ng","summary","sup","table","tbody","td","textarea","tfoot","th","thead","time",
"tr","ul","var","video"],SVG_TAGS=["defs","g","marker","mask","pattern","svg","s\
witch","symbol","feBlend","feColorMatrix","feComponentTransfer","feComposite","f\
eConvolveMatrix","feDiffuseLighting","feDisplacementMap","feFlood","feGaussianBl\
ur","feImage","feMerge","feMorphology","feOffset","feSpecularLighting","feTile",
"feTurbulence","linearGradient","radialGradient","stop","circle","ellipse","imag\
e","line","path","polygon","polyline","rect","text","use","textPath","tspan","fo\
reignObject","clipPath"],TAGS=[...HTML_TAGS,...SVG_TAGS],MEDIA_FEATURES=["any-ho\
ver","any-pointer","aspect-ratio","color","color-gamut","color-index","device-as\
pect-ratio","device-height","device-width","display-mode","forced-colors","grid",
"height","hover","inverted-colors","monochrome","orientation","overflow-block","\
overflow-inline","pointer","prefers-color-scheme","prefers-contrast","prefers-re\
duced-motion","prefers-reduced-transparency","resolution","scan","scripting","up\
date","width","min-width","max-width","min-height","max-height"].sort().reverse(),
PSEUDO_CLASSES=["active","any-link","blank","checked","current","default","defin\
ed","dir","disabled","drop","empty","enabled","first","first-child","first-of-ty\
pe","fullscreen","future","focus","focus-visible","focus-within","has","host","h\
ost-context","hover","indeterminate","in-range","invalid","is","lang","last-chil\
d","last-of-type","left","link","local-link","not","nth-child","nth-col","nth-la\
st-child","nth-last-col","nth-last-of-type","nth-of-type","only-child","only-of-\
type","optional","out-of-range","past","placeholder-shown","read-only","read-wri\
te","required","right","root","scope","target","target-within","user-invalid","v\
alid","visited","where"].sort().reverse(),PSEUDO_ELEMENTS=["after","backdrop","b\
efore","cue","cue-region","first-letter","first-line","grammar-error","marker","\
part","placeholder","selection","slotted","spelling-error"].sort().reverse(),ATTRIBUTES=[
"accent-color","align-content","align-items","align-self","alignment-baseline","\
all","anchor-name","animation","animation-composition","animation-delay","animat\
ion-direction","animation-duration","animation-fill-mode","animation-iteration-c\
ount","animation-name","animation-play-state","animation-range","animation-range\
-end","animation-range-start","animation-timeline","animation-timing-function","\
appearance","aspect-ratio","backdrop-filter","backface-visibility","background",
"background-attachment","background-blend-mode","background-clip","background-co\
lor","background-image","background-origin","background-position","background-po\
sition-x","background-position-y","background-repeat","background-size","baselin\
e-shift","block-size","border","border-block","border-block-color","border-block\
-end","border-block-end-color","border-block-end-style","border-block-end-width",
"border-block-start","border-block-start-color","border-block-start-style","bord\
er-block-start-width","border-block-style","border-block-width","border-bottom",
"border-bottom-color","border-bottom-left-radius","border-bottom-right-radius","\
border-bottom-style","border-bottom-width","border-collapse","border-color","bor\
der-end-end-radius","border-end-start-radius","border-image","border-image-outse\
t","border-image-repeat","border-image-slice","border-image-source","border-imag\
e-width","border-inline","border-inline-color","border-inline-end","border-inlin\
e-end-color","border-inline-end-style","border-inline-end-width","border-inline-\
start","border-inline-start-color","border-inline-start-style","border-inline-st\
art-width","border-inline-style","border-inline-width","border-left","border-lef\
t-color","border-left-style","border-left-width","border-radius","border-right",
...
p-style","border-top-width","border-width","bottom","box-align","box-decoration-\
break","box-direction","box-flex","box-flex-group","box-lines","box-ordinal-grou\
p","box-orient","box-pack","box-shadow","box-sizing","break-after","break-before",
"break-inside","caption-side","caret-color","clear","clip","clip-path","clip-rul\
e","color","color-interpolation","color-interpolation-filters","color-profile","\
color-rendering","color-scheme","column-count","column-fill","column-gap","colum\
n-rule","column-rule-color","column-rule-style","column-rule-width","column-span",
"column-width","columns","contain","contain-intrinsic-block-size","contain-intri\
nsic-height","contain-intrinsic-inline-size","contain-intrinsic-size","contain-i\
ntrinsic-width","container","container-name","container-type","content","content\
-visibility","counter-increment","counter-reset","counter-set","cue","cue-after",
"cue-before","cursor","cx","cy","direction","display","dominant-baseline","empty\
-cells","enable-background","field-sizing","fill","fill-opacity","fill-rule","fi\
lter","flex","flex-basis","flex-direction","flex-flow","flex-grow","flex-shrink",
"flex-wrap","float","flood-color","flood-opacity","flow","font","font-display","\
font-family","font-feature-settings","font-kerning","font-language-override","fo\
nt-optical-sizing","font-palette","font-size","font-size-adjust","font-smooth","\
font-smoothing","font-stretch","font-style","font-synthesis","font-synthesis-pos\
ition","font-synthesis-small-caps","font-synthesis-style","font-synthesis-weight",
"font-variant","font-variant-alternates","font-variant-caps","font-variant-east-\
asian","font-variant-emoji","font-variant-ligatures","font-variant-numeric","fon\
t-variant-position","font-variation-settings","font-weight","forced-color-adjust",
"gap","glyph-orientation-horizontal","glyph-orientation-vertical","grid","grid-a\
rea","grid-auto-columns","grid-auto-flow","grid-auto-rows","grid-column","grid-c\
olumn-end","grid-column-start","grid-gap","grid-row","grid-row-end","grid-row-st\
art","grid-template","grid-template-areas","grid-template-columns","grid-templat\
e-rows","hanging-punctuation","height","hyphenate-character","hyphenate-limit-ch\
ars","hyphens","icon","image-orientation","image-rendering","image-resolution","\
ime-mode","initial-letter","initial-letter-align","inline-size","inset","inset-a\
rea","inset-block","inset-block-end","inset-block-start","inset-inline","inset-i\
nline-end","inset-inline-start","isolation","justify-content","justify-items","j\
ustify-self","kerning","left","letter-spacing","lighting-color","line-break","li\
ne-height","line-height-step","list-style","list-style-image","list-style-positi\
on","list-style-type","margin","margin-block","margin-block-end","margin-block-s\
tart","margin-bottom","margin-inline","margin-inline-end","margin-inline-start",
"margin-left","margin-right","margin-top","margin-trim","marker","marker-end","m\
arker-mid","marker-start","marks","mask","mask-border","mask-border-mode","mask-\
border-outset","mask-border-repeat","mask-border-slice","mask-border-source","ma\
sk-border-width","mask-clip","mask-composite","mask-image","mask-mode","mask-ori\
gin","mask-position","mask-repeat","mask-size","mask-type","masonry-auto-flow","\
math-depth","math-shift","math-style","max-block-size","max-height","max-inline-\
size","max-width","min-block-size","min-height","min-inline-size","min-width","m\
ix-blend-mode","nav-down","nav-index","nav-left","nav-right","nav-up","none","no\
rmal","object-fit","object-position","offset","offset-anchor","offset-distance",
"offset-path","offset-position","offset-rotate","opacity","order","orphans","out\
line","outline-color","outline-offset","outline-style","outline-width","overflow",
"overflow-anchor","overflow-block","overflow-clip-margin","overflow-inline","ove\
rflow-wrap","overflow-x","overflow-y","overlay","overscroll-behavior","overscrol\
l-behavior-block","overscroll-behavior-inline","overscroll-behavior-x","overscro\
ll-behavior-y","padding","padding-block","padding-block-end","padding-block-star\
t","padding-bottom","padding-inline","padding-inline-end","padding-inline-start",
"padding-left","padding-right","padding-top","page","page-break-after","page-bre\
ak-before","page-break-inside","paint-order","pause","pause-after","pause-before",
"perspective","perspective-origin","place-content","place-items","place-self","p\
ointer-events","position","position-anchor","position-visibility","print-color-a\
djust","quotes","r","resize","rest","rest-after","rest-before","right","rotate",
"row-gap","ruby-align","ruby-position","scale","scroll-behavior","scroll-margin",
"scroll-margin-block","scroll-margin-block-end","scroll-margin-block-start","scr\
oll-margin-bottom","scroll-margin-inline","scroll-margin-inline-end","scroll-mar\
gin-inline-start","scroll-margin-left","scroll-margin-right","scroll-margin-top",
"scroll-padding","scroll-padding-block","scroll-padding-block-end","scroll-paddi\
ng-block-start","scroll-padding-bottom","scroll-padding-inline","scroll-padding-\
inline-end","scroll-padding-inline-start","scroll-padding-left","scroll-padding-\
right","scroll-padding-top","scroll-snap-align","scroll-snap-stop","scroll-snap-\
type","scroll-timeline","scroll-timeline-axis","scroll-timeline-name","scrollbar\
-color","scrollbar-gutter","scrollbar-width","shape-image-threshold","shape-marg\
in","shape-outside","shape-rendering","speak","speak-as","src","stop-color","sto\
p-opacity","stroke","stroke-dasharray","stroke-dashoffset","stroke-linecap","str\
oke-linejoin","stroke-miterlimit","stroke-opacity","stroke-width","tab-size","ta\
ble-layout","text-align","text-align-all","text-align-last","text-anchor","text-\
combine-upright","text-decoration","text-decoration-color","text-decoration-line",
"text-decoration-skip","text-decoration-skip-ink","text-decoration-style","text-\
decoration-thickness","text-emphasis","text-emphasis-color","text-emphasis-posit\
ion","text-emphasis-style","text-indent","text-justify","text-orientation","text\
-overflow","text-rendering","text-shadow","text-size-adjust","text-transform","t\
ext-underline-offset","text-underline-position","text-wrap","text-wrap-mode","te\
xt-wrap-style","timeline-scope","top","touch-action","transform","transform-box",
"transform-origin","transform-style","transition","transition-behavior","transit\
ion-delay","transition-duration","transition-property","transition-timing-functi\
on","translate","unicode-bidi","user-modify","user-select","vector-effect","vert\
ical-align","view-timeline","view-timeline-axis","view-timeline-inset","view-tim\
eline-name","view-transition-name","visibility","voice-balance","voice-duration",
"voice-family","voice-pitch","voice-range","voice-rate","voice-stress","voice-vo\
lume","white-space","white-space-collapse","widows","width","will-change","word-\
break","word-spacing","word-wrap","writing-mode","x","y","z-index","zoom"].sort().
reverse();function css(hljs){let regex=hljs.regex,modes=MODES(hljs),VENDOR_PREFIX={
begin:/-(webkit|moz|ms|o)-(?=[a-z])/},AT_MODIFIERS="and or not only",AT_PROPERTY_RE=/@-?\w[\w]*(-\w+)*/,
IDENT_RE3="[a-zA-Z-][a-zA-Z0-9_-]*",STRINGS=[hljs.APOS_STRING_MODE,hljs.QUOTE_STRING_MODE];
return{name:"CSS",case_insensitive:!0,illegal:/[=|'\$]/,keywords:{keyframePosition:"\
from to"},classNameAliases:{keyframePosition:"selector-tag"},contains:[modes.BLOCK_COMMENT,
...
caret()),root,parent,declarations),declarations),(token(previous||1)==5||token(peek()||
1)==5)&&strlen(characters2)&&substr(characters2,-1,void 0)!==" "&&(characters2+=
" ");break;default:characters2+="/"}break;case 123*variable:points[index++]=strlen(
characters2)*ampersand;case 125*variable:case 59:case 0:switch(character2){case 0:case 125:
scanning=0;case 59+offset:ampersand==-1&&(characters2=replace(characters2,/\f/g,
"")),property>0&&(strlen(characters2)-length2||variable===0&&previous===47)&&append(
property>32?declaration(characters2+";",rule,parent,length2-1,declarations):declaration(
replace(characters2," ","")+";",rule,parent,length2-2,declarations),declarations);
break;case 59:characters2+=";";default:if(append(reference=ruleset(characters2,root,
parent,index,offset,rules,points,type,props=[],children=[],length2,rulesets),rulesets),
character2===123)if(offset===0)parse(characters2,root,reference,reference,props,
rulesets,length2,points,children);else switch(atrule===99&&charat(characters2,3)===
110?100:atrule){case 100:case 108:case 109:case 115:parse(value,reference,reference,
rule&&append(ruleset(value,reference,reference,0,0,rules,points,type,rules,props=
[],length2,children),children),rules,children,length2,points,rule?props:children);
break;default:parse(characters2,reference,reference,reference,[""],children,0,points,
children)}}index=offset=property=0,variable=ampersand=1,type=characters2="",length2=
pseudo;break;case 58:length2=1+strlen(characters2),property=previous;default:if(variable<
1){if(character2==123)--variable;else if(character2==125&&variable++==0&&prev()==
125)continue}switch(characters2+=from(character2),character2*variable){case 38:ampersand=
offset>0?1:(characters2+="\f",-1);break;case 44:points[index++]=(strlen(characters2)-
1)*ampersand,ampersand=1;break;case 64:peek()===45&&(characters2+=delimit(next())),
atrule=peek(),offset=length2=strlen(type=characters2+=identifier(caret())),character2++;
break;case 45:previous===45&&strlen(characters2)==2&&(variable=0)}}return rulesets}
__name(parse,"parse");function ruleset(value,root,parent,index,offset,rules,points,type,props,children,length2,siblings){
...
super.disconnectedCallback()}}}__name(IsolatorMixin,"IsolatorMixin");var AdapterClassController=class{constructor(){this.cssStyleSheet=new CSSStyleSheet;
this.styles=[]}static{__name(this,"AdapterClassController")}get style(){return this.
styles.join(`
`)}set style(style){this.styles=[style],this.tagName&&this.cssStyleSheet.replaceSync(
this.adapterClass.cssProcess(`${this.tagName} { ${this.allStyle} }`))}get allStyles(){
let superClass=Object.getPrototypeOf(this.adapterClass),allStyles=[];for(;superClass.
adapter;)allStyles.push(...superClass.adapter.styles),superClass=Object.getPrototypeOf(
superClass);return allStyles.push(...this.styles),allStyles}get allStyle(){return this.
allStyles.join(`
...
allStyle} }`))}addStyle(style){if(this.styles.push(style),this.tagName){let rule=`${this.
tagName} { ${style} }`,processedCss=this.adapterClass.cssProcess(rule);this.cssStyleSheet.
replaceSync(`
        ${this.tagName} { ${this.allStyle} }
        ${processedCss}
      `)}}},AdapterObjectController=class{constructor(){this.cssStyleSheet=new CSSStyleSheet}static{
__name(this,"AdapterObjectController")}get uuid(){return this._uuid?this._uuid:(this.
_uuid=`${this.adapterObject.tagName}-${uuid()}`,this._uuid)}get cssObserver(){return this.
...
super(...args);this._adapter=new AdapterObjectController;this._adapter.adapterObject=
this,this._adapter._class||this._adapter.initClass(),this._adapter.cssObserve(!0)}static{
__name(this,"_Adapter")}static get adapter(){return this._adapter===Object.getPrototypeOf(
this)._adapter&&(this._adapter=new AdapterClassController,this._adapter.adapterClass=
this),this._adapter}static cssProcess(css5){return css5}static set css(css5){this.
adapter.style=css5}static get css(){return this.adapter.style}static get tagName(){
return this.adapter.tagName}static addStyle(css5){this.adapter.addStyle(css5)}static define(tagName){
